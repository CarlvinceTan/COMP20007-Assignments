/*
    ...
    Modified by Grady Fitzpatrick and Carlvince Tan
...
void insert(struct pq *q, int item, double long priority) {
    struct pqNode n;
    n.item = item;
    n.priority = priority;
...
int pull(struct pq *q) {
    int itemPull = q->nodes->item;
    (q->totalItems)--;
